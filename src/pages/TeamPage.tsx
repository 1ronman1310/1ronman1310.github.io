...
const StudentCard: React.FC<{ member: Member, showBio?: boolean, showEmail?: boolean }> = ({ member, showBio = true, showEmail = true }) => (
  <div className="bg-gray-50 rounded-xl p-6 flex flex-col items-center text-center hover:shadow-md transition-shadow h-full border border-transparent hover:border-gray-100">
     ...
     {/* Display Current Position (e.g. for Alumni) */}
     {member.currentPosition && (
        <p className="text-sm font-medium text-apple-blue mb-4 px-2">
          {member.currentPosition}
        </p>
     )}
...
const TeamPage: React.FC = () => {
  ...
  const postdocs = MEMBERS.filter(m => m.role === Role.POSTDOC);
  ...
  return (
    <div className="pt-20 min-h-screen bg-white">
      <Section id="full-team" title="Our Team" subtitle="The researchers and students who make it happen.">
        ...
        {/* Postdoctoral Researchers (Conditional Rendering) */}
        {postdocs.length > 0 && (
          <div className="mb-20">
             <h3 className="text-2xl font-semibold text-apple-dark mb-8 border-b border-gray-100 pb-2">Postdoctoral Researchers</h3>
        ...
        {alumni.length > 0 && (
          <div className="mb-12">
             ...
             {/* Alumni displayed in a slightly denser grid. Bio and Email are hidden via props, but currentPosition will show if present. */}
